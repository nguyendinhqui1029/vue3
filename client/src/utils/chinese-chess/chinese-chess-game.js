import { CHESS_TYPE, COL, ROW } from "./chinese-chess.constant";
...
export class ChineseChessGame {
  BOX_BLACK = CHESS_TYPE.BOX_BLACK;
  BOX_RED = CHESS_TYPE.BOX_RED;
  BOX_EMPTY = CHESS_TYPE.BOX_EMPTY;
  col = COL;
  row = ROW;
  chessBoard = [];
  chessSelected = null;
  ...
  moveTo(chessBox) {
    if (this.chessSelected) {
      ...
      if (chessBox.chessType === this.chessSelected.chessType) {
        ...
        this.setMarkMove(chessBox);
      } else if (chessBox.chessType !== this.chessSelected.chessType && this.chessSelected.isMove(this.chessSelected,chessBox, this.chessBoard)) {
  ...
  setMarkMove(chessBox) {
    if(chessBox.chessType === this.BOX_BLACK) {
  ...
  clearMarkMove() {
    for (let i = 0; i < this.col; i++) {
      for (let j = 0; j < this.row; j++) {
        this.chessBoard[i][j].markMove = false;
      }
    }
  }
  // evaluateBoard(board) {
